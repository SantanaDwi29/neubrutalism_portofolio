...
const ProjectDetailContent = ({ id }: { id: string | undefined }) => {
  ...
  if (!project) return (
    <main id="main-content" className="section-wrap project-not-found">
      <h1>Project not found.</h1><p>This project isn't available. Explore the portfolio to find another one.</p>
  ...
  const changeImage = (direction: number) => setCurrentImageIndex(index => (index + direction + project.images.length) % project.images.length);
    ...
    <main id="main-content" className="section-wrap project-detail">
      ...
      <section className="anime-card detail-gallery" aria-label="Project screenshots">
        <div className="detail-image">
          {failedImage === currentImage ? <div className="gallery-error" role="status"><p>This screenshot could not be loaded.</p><button type="button" className="anime-btn-secondary" onClick={() => setFailedImage(null)}>Try again</button></div> :
        ...
        <div className="gallery-toolbar"><p aria-live="polite">Screenshot {currentImageIndex + 1} of {project.images.length}</p>
          {project.images.length > 1 && <div><button type="button" className="icon-button" aria-label="Previous image" onClick={() => changeImage(-1)}><ChevronLeft size={20} /></button>
            <button type="button" className="icon-button" aria-label="Next image" onClick={() => changeImage(1)}><ChevronRight size={20} /></button></div>}
        </div>
        {project.images.length > 1 && <div className="gallery-thumbnails" role="group" aria-label="Choose a screenshot">
          {project.images.map((image, index) => <button type="button" key={image} aria-label={`View screenshot ${index + 1}`} aria-pressed={index === currentImageIndex} onClick={() => setCurrentImageIndex(index)}>
      ...
      <div className="detail-content">
        ...
        <div className="detail-story"><section><h2>What it does.</h2><p>{project.description}</p></section>
