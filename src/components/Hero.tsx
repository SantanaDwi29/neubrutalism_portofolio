...
export const Hero: React.FC = () => {
  ...
  useEffect(() => {
    // Delay typing animation to allow fast initial render (improves LCP)
    const initTimer = setTimeout(() => setStartTyping(true), 800);
  ...
  }, [currentText, isDeleting, currentRoleIndex, startTyping]);
    ...
    <div className="relative">
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        <div className="md:col-span-8 space-y-8 md:-rotate-1">
          ...
          <h1 className="font-headline-xl text-[48px] leading-tight sm:text-[64px] md:text-headline-xl uppercase min-h-[240px] sm:min-h-[192px]">
            Hi, <br className="hidden sm:block" />I'm  Santa <br className="hidden sm:block" />
            <span className="relative inline-block text-left min-h-[2.4em] sm:min-h-[1.2em] w-[280px] sm:w-[450px] md:w-[600px] align-top mt-2 sm:mt-0">
              <span className="bg-primary-container px-2">{currentText}</span>
              <span className="inline-block w-4 sm:w-6 h-[36px] sm:h-[48px] md:h-[60px] bg-black animate-pulse ml-2 align-middle"></span>
              <svg className="absolute -bottom-2 left-0 w-[80%] sm:w-[90%] h-2 opacity-70" viewBox="0 0 100 20" preserveAspectRatio="none">
